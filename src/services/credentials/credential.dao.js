...
dao.activateCredential = function (id, type) {
  return db.hsetAsync(config.systemConfig.db.redis.namespace.concat('-', type).concat(':', id), 'isActive', 'true');
};

dao.deactivateCredential = function (id, type) {
  return db.hsetAsync(config.systemConfig.db.redis.namespace.concat('-', type).concat(':', id), 'isActive', 'false');
};
