...
const namespace = 'express-gateway:credentials:remove-scopes';
const idGen = require('uuid-base62');

describe('eg credentials remove-scopes', () => {
  ...
  it('remove-scopes from a credential', done => {
    ...
    env.argv = program.parse(`credentials remove-scopes --id ${cred1.keyId} -t key-auth ${scope1} ${scope2}`);
  });
  ...
  it('prints only the credential id when using the --quiet flag', done => {
    ...
    env.argv = program.parse(`credentials remove-scopes -t key-auth -q --id ${cred1.keyId} ${scope1} ${scope2}`);
  });
});
