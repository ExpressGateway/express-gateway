  ...
  let client = new Proxy(superagent, {
    get: function (request, name) {
      if (methods.indexOf(name) >= 0) {
        ...
        return (url, query = {}) => {
          const queryString = _.map(query, (value, key) => `${key}=${encodeURIComponent(value)}`).join('&');
          const currentUrl = baseUrl + url + (queryString ? `?${queryString}` : '');
          debug('>', method.toUpperCase(), currentUrl);
          const relativeUrlRequest = new Proxy(request(method, currentUrl), wrapHandler);
          if (headers) {
            return relativeUrlRequest.set(prepareHeaders(headers));
          }
