...
const {resolveSome} = require('../utils');
...
dao.find = function (tokenObj, options) {
  ...
  return db.hgetallAsync(consumerTokensKey)
    .then((tokenIds) => {
      ...
      tokenPromises = activeTokenIds.map(async (id) => {
        const token = await db.hgetallAsync(tokenNamespace.concat(':', id));
        if (!token) {
          throw new Error();
        }

        if (!Object.keys(tokenObj).every((key) => tokenObj[key] === token[key])) {
          throw new Error();
        }
        return token;
      });
      ...
      } else getTokenPromise = resolveSome(tokenPromises);
...
dao.get = function (tokenId, options) {
  ...
  let tokenNamespace;
  let type = options.type || 'access_token';
  tokenNamespace = redisNamespace.concat('-', type === 'access_token' ? accessTokenNamespace : refreshTokenNamespace);
