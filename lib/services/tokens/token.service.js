...
s.createJWT = function (payload) {
  const accessTokenConfig = config.systemConfig.accessTokens;
  const secret = accessTokenConfig.secretOrPrivateKeyFile ? fs.readFileSync(accessTokenConfig.secretOrPrivateKeyFile) : accessTokenConfig.secretOrPrivateKey;

  return new Promise((resolve, reject) => {
    jwt.sign(payload, secret, {
      issuer: accessTokenConfig.issuer,
      audience: accessTokenConfig.audience,
      expiresIn: accessTokenConfig.timeToExpiry,
      subject: accessTokenConfig.subject
    }, (err, jwt) => {
