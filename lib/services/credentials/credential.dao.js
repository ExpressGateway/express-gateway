 ...
 * 
 * @id {String}
 * @deleteAll {Boolean} Optional - Whether to delete the key / set that contains all creds for users.
 */
dao.removeAllCredentials = function (id, deleteAll) {
  ...
  const awaitAllPromises = credentialTypes.map((type) => {
    let authKey = buildIdKey(type, id);
    ...
    if (type === 'key-auth') {
      ...
      return db.smembers(authKey)
      .then(ids => {
        
        // Delete each 
        let promises = ids.map(keyId => {
          return dbTransaction.del(buildIdKey(type, keyId));
        });
