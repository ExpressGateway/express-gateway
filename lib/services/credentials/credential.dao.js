...
dao.insertScopes = function (_scopes) {
  ...
  return db.hmset.apply(db, db.prepareHMSET(scopeDbKey, scopes));
};
        ...
        .then(credentialObjs => {
          ...
          credentialObjs.forEach((credentialObj, index) => {
            ...
            if (credentialObj && credentialObj.scopes) {
              ...
              removeScopesTransaction = removeScopesTransaction.hmset.apply(removeScopesTransaction, db.prepareHMSET(credentialId, { scopes: JSON.stringify(newScopes) }));
            }
          });
...
dao.insertCredential = function (id, type, credentialObj) {
  ...
  if (type === 'key-auth') {
    return Promise.all([
      ...
      db.hmset.apply(db, db.prepareHMSET(key, credentialObj))
    ]);
  }
  return db.hmset.apply(db, db.prepareHMSET(key, credentialObj));
};
