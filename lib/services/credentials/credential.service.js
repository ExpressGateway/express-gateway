...
const uuid62 = require('uuid62'); // special format for uuid, url friendly base62 encoding
  ...
  return credentialDao.getCredential(id, type)
    .then(credential => {
      ...
      if (!options || !options.includePassword) {
        delete credential[config.models.credentials.properties[type].passwordKey];
      }
      return credential;
    });
};
