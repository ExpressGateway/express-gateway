...
s.insertCredential = function (id, type, credentialDetails) {
  ...
  let credentialExistsPromise;
  if (areMultipleCredsAllowed) {
    credentialExistsPromise = Promise.resolve(dereferencePromise);
  } else {
    // check if credential already exists
    credentialExistsPromise = this.getCredential(id, type)
      .then(cred => {
  ...
  return credentialExistsPromise.then((resolvedSchema) => {
