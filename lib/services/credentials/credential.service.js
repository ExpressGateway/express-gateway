...
let uuid62 = require('uuid-base62'); // special format for uuid, url friendly base62 encoding
...
s.getCredential = function (query, type, options) {
  ...
  return credentialDao.getCredential(query, type)
    .then(credential => {
      if (!credential) {
        return null;
      }
