...
s.insertCredential = function (id, type, credentialDetails) {
  ...
  return this.getCredential(id, type) // check if credential already exists
    .then((cred) => {
      ...
      return refParser.dereference(config.models.credentials);
    })
    .then((resolvedSchema) => {
      const credentialConfig = mergeAllOf(resolvedSchema).properties[type];
  ...
  return credentialDao.getCredential(id, type)
    .then(credential => {
      ...
      if (!options || !options.includePassword) {
        delete credential[config.models.credentials.properties[type].properties.passwordKey.default];
      }
      return credential;
    });
};
