...
dao.insert = function (app) {
  ...
  const appNameSetKey = config.systemConfig.db.redis.namespace.concat('-', appnameNamespace).concat(':', app.name);

  return db
    .multi()
    .hmset(appHashKey, app)
    .sadd(userAppsHashKey, app.id)
    .sadd(appNameSetKey, app.id)
    .execAsync()
...
dao.find = function (appName) {
  return db.smembersAsync(config.systemConfig.db.redis.namespace.concat('-', appnameNamespace).concat(':', appName))
    .then(function (Ids) {
      if (Ids && Ids.length !== 0) {
        return Ids[0];
      } else return false;
