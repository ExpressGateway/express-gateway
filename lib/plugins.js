...
const { getPipelineRouter } = require('./gateway/pipelines');
...
module.exports.load = function ({ config }) {
  ...
  // TODO: this is a temp solution until everything will be refactored into plugins
  pluginsSettings['run-pipeline'] = {
    package: './core-plugins/run-pipeline'
  };
  ...
  for (const pluginName in pluginsSettings) {
    ...
    try {
      ...
      try {
        ...
        logger.error(`Wrong schema definition for ${pluginName}`);
      }
