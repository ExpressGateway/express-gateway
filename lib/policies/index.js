...
const load = (policiesToBeLoaded) => {
  ...
  const policiesWithoutPlugins = policiesToBeLoaded.filter((name) => policyNames.includes(name));

  policiesWithoutPlugins.forEach((name) => {
...
module.exports = { register, resolve, policies, load, removeAll };
