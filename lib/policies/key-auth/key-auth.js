...
module.exports = function (params) {
  ...
  passport.use(strategyName, new LocalAPIKeyStrategy(Object.assign({ passReqToCallback: true }, params), (req, apikeyHeaderValue, done) => {
    // key will look like "h1243h1kl23h4kjh:asfasqwerqw"
    if (!apikeyHeaderValue) {
      return done(null, false);
    }

    const apiKeyValueParts = apikeyHeaderValue.split(' ');
    const keyParts = apiKeyValueParts[apiKeyValueParts.length - 1].split(':');
