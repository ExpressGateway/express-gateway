...
module.exports = function (actionParams) {
  ...
  passport.use(`bearer-introspect_${actionParams.endpoint}`, new BearerStrategy({ passReqToCallback: true }, (req, accessToken, done) => {
  ...
  return (req, res, next) => {
    passport.authenticate(`bearer-introspect_${actionParams.endpoint}`, actionParams, actionParams.getCommonAuthCallback(req, res, next))(req, res, next);
  };
};
