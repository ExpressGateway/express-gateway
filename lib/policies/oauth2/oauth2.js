'use strict';
const omitedProps = ['tokenDecrypted'];
...
function setHeaders (req, headerObject, namespace) {
  for (let key in headerObject) {
    // TODO: this is proxy concern to set HEADERS,
    // in this policy we should only declare information that somehow can be sent downstream
    // (most reasonable way is indeed use heders, but it is not the only option)
    // some properties should not be proxied due to security reasons
    if (omitedProps.some(p => p === key)) {
      continue;
    }
    const val = headerObject[key];
    if (val) {
