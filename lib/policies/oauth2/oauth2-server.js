...
module.exports.authorization = [
  ...
  server.authorization((clientID, redirectURI, scope, done) => {
    return authService.validateConsumer(clientID)
      .then(consumer => {
        ...
        return authService.authorizeCredential(clientID, 'oauth2', scope)
          .then(authorized => {
            if (!authorized) {
              return done(null, false);
            }
  ...
  (req, res) => {
    ...
    res.render(path.join(__dirname, 'views/dialog'), { transactionId: req.oauth2.transactionID, user: req.user, client: req.oauth2.client });
  }
];
