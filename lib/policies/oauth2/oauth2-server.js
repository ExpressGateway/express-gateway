...
server.exchange(oauth2orize.exchange.password((consumer, username, password, scopes, done) => {
  ...
  return authService.validateConsumer(consumer.id)
    ...
    .then(user => {
      ...
      return Promise.all([user, scopes ? authService.checkScopesOnCredential(consumer.id, 'oauth2', scopes) : true]);
    })
...
server.exchange(oauth2orize.exchange.clientCredentials((consumer, scopes, done) => {
  ...
  return authService.validateConsumer(consumer.id)
    .then(consumer => {
      ...
      if (scopes) {
        scopeAuthorizationPromise = authService.checkScopesOnCredential(consumer.id, 'oauth2', scopes);
  ...
  server.authorization((clientID, redirectURI, scope, done) => {
    return authService.validateConsumer(clientID)
      .then(consumer => {
        ...
        return authService.checkScopesOnCredential(clientID, 'oauth2', scope)
          .then(authorized => {
            if (!authorized) {
              return done(null, false);
            }
