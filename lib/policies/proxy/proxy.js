...
// Extend proxy options by allowing and parsing keyFile and certFile.
function parseProxyOptions (proxyOptions = {}, endpoint) {
  let parsedProxyOptions;

  if (Object.keys(endpoint).includes('proxyOptions')) {
    parsedProxyOptions = Object.assign(endpoint.proxyOptions, proxyOptions);
  }

  if (!Object.keys(parsedProxyOptions).includes('target')) parsedProxyOptions.target = {};

  if (endpoint.url) {
    const parsedEndpointUrl = endpoint.url ? url.parse(endpoint.url) : {};

    parsedProxyOptions.target = Object.assign(parsedProxyOptions.target, {
      host: parsedEndpointUrl.host,
      port: parsedEndpointUrl.port,
      protocol: parsedEndpointUrl.protocol,
      path: parsedEndpointUrl.path
    });
  }
