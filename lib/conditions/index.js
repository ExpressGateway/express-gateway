...
function register({ type = 'condition', name, handler, schema, middlewares }) {
  ...
  conditions[name] = config => {
    ...
    if (validationResult.isValid) {
      if (handler.length === 2) {
        return { middlewares, handler: req => handler(req, config) };
      }

      return { middlewares, handler: handler(config) };
    }
