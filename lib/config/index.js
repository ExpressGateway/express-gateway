...
Config.prototype.watchConfigFiles = function () {
  const systemConfigWatcher = fs.watch(config.systemConfigPath, (evt, name) => {
    if (evt !== 'change') {
      return;
    }
  ...
  const gatewayConfigWatcher = fs.watch(config.gatewayConfigPath, (evt, name) => {
    if (evt !== 'change') {
      return;
    }
  ...
  this.watchers.push(systemConfigWatcher);
  this.watchers.push(gatewayConfigWatcher);
};

Config.prototype.closeFileWatchers = function () {
