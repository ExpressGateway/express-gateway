...
Config.prototype.watchConfigFiles = function () {
  const watchOptions = { persistent: false };

  fs.watch(config.systemConfigPath, watchOptions, (evt, name) => {
    if (evt !== 'change') {
      return;
    }

    log.info(`${evt} event on ${name} file. Reloading system config file ` +
      `from ${config.systemConfigPath}`);

    config.loadSystemConfig();
    config.emitter.emit('systemConfigChange');
  });

  fs.watch(config.gatewayConfigPath, watchOptions, (evt, name) => {
    if (evt !== 'change') {
      return;
    }

    log.info(`${evt} event on ${name} file. Reloading gateway config file ` +
      `from ${config.gatewayConfigPath}`);
