...
class Config {
  ...
  watch () {
    this.watcher = nsfw(this.gatewayConfigPath, events => {
      events.forEach(event => {
        if (event.action !== nsfw.actions.MODIFIED) return;

        log.info(`MODIFIED event on ${this.gatewayConfigPath} file. Reloading gateway config file`);
        try {
  ...
  unwatch () {
    if (this.watcher) { return this.watcher.then(w => w.stop()); } else return Promise.resolve();
  }
