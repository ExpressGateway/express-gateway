...
class Config {
  constructor () {
    ...
    this.configMap = {
      system: {
        ...
        configValue: 'systemConfig'
      },
      gateway: {
        baseFilename: 'gateway.config',
        validator: this.gatewayValidator,
  ...
  loadConfig (type) {
    ...
    let configV;
    let configPath = this[config.path] || path.join(process.env.EG_CONFIG_DIR, `${config.baseFilename}.yml`);

    try {
      configV = yamlOrJson.load(_envReplace(fs.readFileSync(configPath, 'utf8'), process.env));
    } catch (err) {
      try {
        configPath = path.join(process.env.EG_CONFIG_DIR, `${config.baseFilename}.json`);
    ...
    const { isValid, error } = config.validator(configV);
    ...
    this[config.configValue] = configV;
  ...
  watch () {
    ...
    watchEvents.forEach(watchEvent => {
      systemWatcher.on(watchEvent, name => {
        ...
        try { this.loadConfig('system'); } catch (e) { log.debug('Failed hot reload of system config'); }
      ...
      gatewayWatcher.on(watchEvent, name => {
        ...
        try { this.loadConfig('gateway'); } catch (e) { log.debug('Failed hot reload of gateway config'); }
