...
module.exports = function ({ plugins, config } = {}) {
  ...
  return new Promise(resolve => {
    const callback = () => {
      ...
      console.log(`admin http server listening on ${address}`);
      eventBus.emit('admin-ready', { adminServer: srv });
      resolve(srv);
    };

    const args = [cfg.admin.backlog, callback];

    if (cfg.admin.hostname) {
      args.unshift(cfg.admin.hostname);
    }

    if (typeof (cfg.admin.port) === 'number') {
      args.unshift(cfg.admin.port);
    }

    const srv = app.listen(...args);
  });
};
