...
module.exports = function () {
  ...
  router.get('/', function (req, res, next) {
    Promise.all([
      ...
      Promise.all(users.concat(apps).map(consumer => services.credential.getCredentials(consumer.id)))
    ).then(credentials => res.json({ credentials: [].concat.apply([], credentials) }))
