...
let configuredPipelines = {};

module.exports.getPipelineRouter = (name) => {
  return configuredPipelines[name];
};
module.exports.bootstrap = function ({ app, config }) {
  ...
  for (const name in config.gatewayConfig.pipelines) {
    ...
    configurePipelineTemp[name] = router;
  ...
  // new config is loaded, no exception happend, so we can update pipelines
  configuredPipelines = configurePipelineTemp;
  return app;
};
