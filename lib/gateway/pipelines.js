...
function configurePipeline (pipelinePoliciesConfig, config) {
  ...
  pipelinePoliciesConfig.forEach(policyConfig => {
    ...
    const {policy, schema} = policies.resolve(policyName);
    ...
    try {
      ...
      log.error(`Invalid schema for ${policyName} policy`, err);
      return null;
    }
    ...
    for (const policyStep of policySteps) {
      ...
      try {
        ...
        log.error(`Policy ${policyName} params validation failed`, err);
        return null;
      }
