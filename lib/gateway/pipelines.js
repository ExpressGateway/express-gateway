  ...
  pipelinePoliciesConfig.forEach(policyConfig => {
    ...
    const middlewares = flatMap(policySteps, policyStep => {
      ...
      if (condition instanceof Promise) {
        return condition.then(condFn => conditionAndActionMiddleware(conditionConfig, condFn, policyName, policyMiddleware, middlewares));
      }

      return conditionAndActionMiddleware(conditionConfig, condition, policyName, policyMiddleware, middlewares);
    });
...
function evaluateConditionResult(result, { req, res, next, policyName, policyMiddleware }) {
  if (result) {
...
function conditionAndActionMiddleware(conditionConfig, conditionFunction, policyName, policyMiddleware, middlewares) {
  const fn = (req, res, next) => {
    if (conditionConfig) {
      return evaluateConditionResult(conditionFunction(req), { req, res, policyName, policyMiddleware, next });
    }
    return policyMiddleware(req, res, next);
  };
