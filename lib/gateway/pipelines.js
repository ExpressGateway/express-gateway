...
module.exports.bootstrap = function (app, serverInstances) {
  ...
  for (let name in config.gatewayConfig.pipelines) {
    ...
    let router = configurePipeline(pipeline.policies || [], serverInstances);
...
function configurePipeline (pipelinePoliciesConfig, serverInstances) {
  let router = express.Router();
  ...
  pipelinePoliciesConfig.forEach(policyConfig => {
    ...
    for (let policyStep of policySteps) {
      ...
      const policyMiddleware = policy(action, config, serverInstances);
