...
module.exports = function ({ plugins, config } = {}) {
  ...
  if (config.gatewayConfig.http && httpServer) {
    // config.gatewayConfig.http.hostname = config.gatewayConfig.http.hostname || 'localhost';
    appPromises.push(
      new Promise(resolve => {
        const runningApp = httpServer.listen(config.gatewayConfig.http.port, config.gatewayConfig.http.hostname, () => {
          // This can either be an object or a String depending on whether you specify a host or not
          // https://nodejs.org/api/net.html#net_server_address
          if (config.gatewayConfig.http.hostname) {
            log.info(`gateway http server listening on ${config.gatewayConfig.http.hostname}:${config.gatewayConfig.http.port}`);
          } else {
  ...
  if (config.gatewayConfig.https && httpsServer) {
    // config.gatewayConfig.https.hostname = config.gatewayConfig.https.hostname || 'localhost';
    appPromises.push(
      new Promise(resolve => {
        const runningApp = httpsServer.listen(config.gatewayConfig.https.port, config.gatewayConfig.https.hostname, () => {
          // This can either be an object or a String depending on whether you specify a host or not
          // https://nodejs.org/api/net.html#net_server_address
          if (config.gatewayConfig.https.hostname) {
            log.info(`gateway https server listening on ${config.gatewayConfig.https.hostname}:${config.gatewayConfig.https.port}`);
          } else {
            const { address, port } = runningApp.address();
